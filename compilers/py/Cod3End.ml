      ...
      ( match cmd with
        | CmdInput ((t,p), args) ->
          ...
          let codigo = (List.concat codigos) @
                       ...
                       [Call ("read", [(t,tipo)], TipoNone)]
          in (t, codigo)
        | _ -> failwith "Incorrect exp input"
      )
  | _ -> failwith "traduz_exp: não implementado"


let rec traduz_cmd cmd local =
  match cmd with
  | CmdReturn exp ->
    (match exp with
     | None -> [Return None]
  ...
  | CmdIf (teste, entao, senao) ->
    ...
    and codigo_entao = traduz_cmds entao local
    ...
    (match senao with
        | None -> codigo_teste @ 
                  ...
                  [rotulo_falso]
        | Some cmds -> 
          let codigo_senao = traduz_cmds [cmds] local
  ...
  | CmdWhile (cond, corpo) -> 
      let (endr_teste, codigo_teste) = traduz_exp cond
      and codigo_verdadeiro = traduz_cmds corpo local
  ...
  | CmdElse cmds -> traduz_cmds cmds local

  | CmdVarDec (e, t) ->
    let id = (
      match e with
       | ExpVar (ev,_) -> (match ev with VarSimples id -> fst id)
       | _ -> failwith "CmdVarDec") in
  ...
  | CmdVarDecAtrib (e, t, valor) ->
    let id = (match e with
      ExpVar (ev,_) -> (match ev with VarSimples id -> fst id)
      | _ -> failwith "CmdVarDecAtrib")   
    in let dec =       
    if local then
      [Local (id,t)]
    else 
      [Global (id,t)] 
    in
    let (endr_exp, codigo_exp) = traduz_exp valor 
    and (endr_elem, codigo_elem) = traduz_exp e in
    dec @ codigo_exp @ codigo_elem @ [Copia (endr_elem, endr_exp)] 
  
  | _ -> failwith "traduz_cmd: não implementado"
    

and traduz_cmds cmds local =
  match cmds with
  | [] -> []
  | cmd :: cmds ->
    let codigo = traduz_cmd cmd local in
    codigo @ traduz_cmds cmds local

let conta_locais corpo =
  List.length (List.filter (fun cmd ->
  match cmd with
    | CmdVarDec (_,_) -> true             
    | CmdVarDecAtrib (_,_,_) -> true
    | _ -> false
  ) corpo ) 

let traduz_ast ast =
  match ast with
    Cmd cmd -> traduz_cmd cmd false
  | Funcao {fn_nome; fn_tiporet; fn_formais; fn_corpo} ->  
      ...
      and corpo = traduz_cmds fn_corpo true
      in
      let nlocais = conta_locais fn_corpo
      in
      [BeginFun (nome,nformais,nlocais)] @ formais @ corpo @ [EndFun]

let tradutor ast_tipada =
  let _ = zera_contadores () in 
  let (Programa inst) = ast_tipada in
  let cmds =    List.map traduz_ast inst in
  List.concat cmds