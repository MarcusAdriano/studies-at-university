...
let obtem_nome_tipo_var exp = let open T in
  match exp with
    ...
    | _                  -> failwith "obtem_nome_tipo_var1: nao é variavel"

let pega_int exp = match exp with
  | T.ExpInt (i,_)  -> i
  | _               -> failwith "pega_int: nao é inteiro"

let pega_float exp = match exp with
  | T.ExpFloat (f,_)-> f
  | _               -> failwith "pega_float: nao é inteiro"

let pega_str exp = match exp with
  | T.ExpStr (s,_)  -> s
  | _               -> failwith "pega_string: nao é string"

let pega_bool exp = match exp with
  | T.ExpBool (b,_) -> b
  | _               -> failwith "pega_bool: nao é booleano"
...
let rec interpreta_exp amb exp = 
  ...
  let open T in
    ...
    | ExpVar (var, tipo) ->
      (
        match var with
          | A.VarSimples (nome, _ (*pos*)) ->
            (match (Amb.busca amb nome) with
              | Amb.EntVar (_, v) ->
                (match v with
                  | Some valor -> valor
                  | None       -> let msg = "variável nao inicializada: " ^ nome in failwith msg
                )
    ...
    | ExpInput (cmd, _) -> 
        (
          try
            let _ = interpreta_cmd amb cmd in failwith "CmdInput inválido"
          with
...
and interpreta_cmd amb cmd =
  ...
  | CmdInput ((tipo,_),exps) ->
        let _ = (
         List.iter (    
           fun exp -> let resp = interpreta_exp amb exp in
             (match resp with
               | T.ExpInt   (n,_) -> print_int    n
               | T.ExpFloat (n,_) -> print_float  n
               | T.ExpStr   (n,_) -> print_string n
               | _ -> failwith "nao foi possivel exibir o valor da variavel (CmdInput)"
         )) exps
        ) in 
        (
           match tipo with
             | TipoInt   -> let e = T.ExpInt   (read_int    (), tipo) in raise (Valor_de_retorno e)
             | TipoStr   -> let e = T.ExpStr   (read_line   (), tipo) in raise (Valor_de_retorno e)
             | TipoFloat -> let e = T.ExpFloat (read_float  (), tipo) in raise (Valor_de_retorno e)
             | _         -> failwith "falha no input"
         )
  | CmdVarDecAtrib (exp, _, expValor) -> 
      (match exp with
        | T.ExpVar (var, tipo) ->
          ( match var with
            | A.VarSimples (id, pos) -> Amb.insere_local amb id tipo (Some (interpreta_exp amb expValor)) 
          )
        | _ -> failwith "Erro ao declarar variavel"
      )
  | CmdVarDec (exp, _) -> 
      (match exp with
        | T.ExpVar (var, tipo) ->
          ( match var with
            | A.VarSimples (id, pos) -> Amb.insere_local amb id tipo None 
          )
        | _ -> failwith "Erro ao declarar variavel"
      )
  ...
  | CmdPrint exps -> 
    ...
    fun exp -> let resp = interpreta_exp amb exp in
      (match resp with
        ...
        | T.ExpFloat (n,_) -> print_float  n
        | T.ExpStr   (n,_) -> print_string n
        | _ -> failwith "nao foi possivel exibir o valor da variavel (CmdPrint)"
      )) exps
  | CmdWhile (cond, cmds) -> 
